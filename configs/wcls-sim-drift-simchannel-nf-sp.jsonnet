...
local wcls_depoflux_writer = g.pnode({
  ...
  data: {
    ...
    sparse: false,
    // SimChannel smearing to SP/reco resolution so the 2D truth labels register onto
    // the deconvolved reco charge (added in quadrature to the post-drift diffusion).
    // dune10kt-1x2x6 SP-filter recipe:
    //   smear_long [raw ticks] = sigma_t/tick, sigma_t = 1/(2*pi*0.12MHz) = 1.326 us
    //   smear_tran [wire pitch] per plane [U,V,W]: induction k=0.75 -> 0.376, collection k=3.0 -> 0.094
    smear_long: 2.6526,
